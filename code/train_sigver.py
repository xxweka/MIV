...
conv_pretrain_model = 'EfficientNetV2B3'
IMAGE_SIZE = (256, 128)  # w, h
MAX_NUM_TARGETS = 8
...
flags.DEFINE_string('train', 'train_sigver1.tsv', 'Train dataset under the root directory')
flags.DEFINE_string('dev', 'dev_sigver1.tsv', 'Dev dataset under the root directory')
# flags.DEFINE_string('test', 'test_sigver1.tsv', 'Test dataset under the root directory')
...
def main(unused_argv):
    ...
    target_seq = None  # MAX_NUM_TARGETS  #
    inshape = {INPUT_KEYS['query']: query_shape, INPUT_KEYS['target']: (target_seq,) + query_shape}
    nheads = 6 if conv_pretrain_model.endswith('V2B3') else 5
    ...
    data_cache = build_images_cache(cache_filename, infile_list=['query/', 'anchors/'])
    ...
    BATCH_SIZE_PER_REPLICA = 16 if model_id > 0 else 24
    ...
    if not is_pre0trained:
        learn_rates = [5e-5, 2e-5, 1e-5]
        boundaries = [1 * batches_per_epoch, 5 * batches_per_epoch]
        ...
        if model_id < 2:
            lr_values = [1e-5, 5e-6]
            boundaries = [5 * batches_per_epoch - 1]
        else:
            lr_values = [5e-6, 2e-6, 1e-6]
            boundaries = [5 * batches_per_epoch - 1, 10 * batches_per_epoch - 1]
        lr_schedule = tk.optimizers.schedules.PiecewiseConstantDecay(
            boundaries=boundaries,
            values=lr_values
        )
    ...
    if latest_ckpt:
        ...
        if is_pre0trained:
            ...
            if not os.path.exists(pre0trained):
                frbsz = num_in_sync * (48 if model_id > 0 else 128)
                frbsz = int(frbsz * batch_size_multiplier)
                bpe = int(math.ceil(train_samplesize / frbsz))
                if model_id < 2:
                    lr_bdr = [5 * bpe - 1, 10 * bpe - 1]
                    lr_val = [1e-4, 5e-5, 2e-5]
                else:
                    lr_bdr = [2 * bpe - 1, 5 * bpe - 1]
                    lr_val = [5e-5, 2e-5, 1e-5]
